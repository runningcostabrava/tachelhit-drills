...
def check_and_fix():
    ...
    if 'drills' not in table_names:
        ...
        if 'author' not in columns:
            print("[SCHEMA] Column 'author' missing. Adding...")
            try:
                with engine.connect() as conn:
                    if DATABASE_URL.startswith('sqlite'):
                        try:
                            conn.execute(text("ALTER TABLE drills ADD COLUMN author VARCHAR"))
                            conn.commit()
                            print("[SCHEMA] Column 'author' added successfully (SQLite).")
                        except Exception as e:
                            if 'duplicate column name' in str(e).lower():
                                print("[SCHEMA] Column 'author' already exists (SQLite).")
                            else:
                                raise
                    else:
                        conn.execute(text("ALTER TABLE drills ADD COLUMN IF NOT EXISTS author VARCHAR"))
                        conn.commit()
                        print("[SCHEMA] Column 'author' added successfully (PostgreSQL).")
            except Exception as e:
                print(f"[SCHEMA] Error adding 'author' column: {e}")
        else:
            print("[SCHEMA] Column 'author' already exists.")
        
        # Check for 'is_correction_dataset' column
        if 'is_correction_dataset' not in columns:
            print("[SCHEMA] Column 'is_correction_dataset' missing. Adding...")
            try:
                with engine.connect() as conn:
                    if DATABASE_URL.startswith('sqlite'):
                        try:
                            conn.execute(text("ALTER TABLE drills ADD COLUMN is_correction_dataset BOOLEAN DEFAULT FALSE"))
                            conn.commit()
                            print("[SCHEMA] Column 'is_correction_dataset' added successfully (SQLite).")
                        except Exception as e:
                            if 'duplicate column name' in str(e).lower():
                                print("[SCHEMA] Column 'is_correction_dataset' already exists (SQLite).")
                            else:
                                raise
                    else:
                        conn.execute(text("ALTER TABLE drills ADD COLUMN IF NOT EXISTS is_correction_dataset BOOLEAN DEFAULT FALSE"))
                        conn.commit()
                        print("[SCHEMA] Column 'is_correction_dataset' added successfully (PostgreSQL).")
            except Exception as e:

#!/usr/bin/env python3
"""
Debug script to test SRT import logic with TTS.
"""
import sys
import os
import tempfile
...
# Mock the generate_catalan_tts function from main.py
def generate_catalan_tts(text: str, drill_id: int) -> str:
    ...
    try:
        ...
        print(f"  [TTS] Generating TTS for text: {text[:50]}...")
        ...
        print(f"  [TTS] Generated TTS URL: {url}")
...
print(f"\n--- Verification ---")
all_drills = db.query(DrillModel).filter(DrillModel.tag == "youtube_srt_test_tts").all()
print(f"Found {len(all_drills)} drills with tag 'youtube_srt_test_tts' in database")
for drill in all_drills:
    print(f"  Drill {drill.id}: {drill.text_catalan[:50]}...")
    if drill.audio_tts_url:
        print(f"    TTS URL: {drill.audio_tts_url}")

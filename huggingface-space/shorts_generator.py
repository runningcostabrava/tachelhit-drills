...
try:
    ...
    if ffmpeg_path:
        print(f"[SHORTS] imageio-ffmpeg reports ffmpeg at: {ffmpeg_path}")
        if os.path.exists(ffmpeg_path):
            # Set environment variable for moviepy to use this ffmpeg
            os.environ["FFMPEG_BINARY"] = ffmpeg_path
            print(f"[SHORTS] Found ffmpeg at: {ffmpeg_path}")
        else:
            print(f"[SHORTS] Warning: ffmpeg path does not exist: {ffmpeg_path}")
    else:
...
try:
    ...
    from moviepy.editor import ImageClip, VideoFileClip, AudioFileClip, CompositeVideoClip, concatenate_videoclips, concatenate_audioclips
...
def generate_youtube_short(drill_data, output_filename):
    ...
    # If there's audio, we'll use its exact duration
    audio_clip = None
    if drill_data.get('audio_url'):
        try:
            ...
            if audio_url.startswith('http'):
                ...
                # CRITICAL FIX: Set duration exactly to audio duration, no extra padding
    ...
    # Create video clip with the determined duration
    video_clip = ImageClip(img_array, duration=duration)

    # Add audio if available
    if audio_clip:
        # Attach audio; set_audio will keep the clip's current duration (already matched)
...
def generate_drillplayer_demo(test_id, drills_data, output_filename):
    ...
    for i, drill in enumerate(drills_data):
        ...
        # Each drill appears for a base duration (e.g., 5 seconds for visual)
        base_clip_duration = 5.0 
        clip = ImageClip(img_array, duration=base_clip_duration)
        
        # --- Audio Orchestration ---
        all_audio_clips_for_drill = []
        current_audio_duration_estimate = 0.0

        # 1. Process Catalan TTS audio (plays once)
        if drill.get('audio_tts_url'):
            try:
                # Assuming audio_tts_url is a direct URL now, not local path
                tts_audio_url = drill['audio_tts_url']
                if tts_audio_url.startswith('http'):
                    import requests
                    import tempfile
                    print(f"[DEMO] Downloading TTS audio from URL: {tts_audio_url}")
                    response = requests.get(tts_audio_url, timeout=15)
                    response.raise_for_status()
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as tmp_file:
                        tmp_file.write(response.content)
                        tmp_tts_path = tmp_file.name
                    
                    tts_clip = AudioFileClip(tmp_tts_path)
                    all_audio_clips_for_drill.append(tts_clip)
                    current_audio_duration_estimate += tts_clip.duration
                else:
                    print(f"[DEMO] Local TTS audio path not supported in Space: {tts_audio_url}")
            except Exception as e:
                print(f"[DEMO] Error loading TTS audio from URL: {e}")

        # 2. Process Tachelhit audio (plays twice)
        if drill.get('audio_url'):
            try:
                # Assuming audio_url is a direct URL now
                tachelhit_audio_url = drill['audio_url']
                if tachelhit_audio_url.startswith('http'):
                    import requests
                    import tempfile
                    print(f"[DEMO] Downloading Tachelhit audio from URL: {tachelhit_audio_url}")
                    response = requests.get(tachelhit_audio_url, timeout=15)
                    response.raise_for_status()
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.mp3') as tmp_file:
                        tmp_file.write(response.content)
                        tmp_tachelhit_path = tmp_file.name
                    
                    tachelhit_clip = AudioFileClip(tmp_tachelhit_path)
                    # Play twice: concatenate the clip with itself
                    doubled_tachelhit_clip = concatenate_audioclips([tachelhit_clip, tachelhit_clip])
                    all_audio_clips_for_drill.append(doubled_tachelhit_clip)
                    current_audio_duration_estimate += doubled_tachelhit_clip.duration
                    
                    # Close individual clips to free resources
                    tachelhit_clip.close()
                else:
                    print(f"[DEMO] Local Tachelhit audio path not supported in Space: {tachelhit_audio_url}")
            except Exception as e:
                print(f"[DEMO] Error loading Tachelhit audio from URL: {e}")
        
        # 3. Combine all audio clips for this drill
        final_drill_audio = None
        if all_audio_clips_for_drill:
            final_drill_audio = concatenate_audioclips(all_audio_clips_for_drill)
            
            # Close individual clips after concatenation to free resources
            for ac in all_audio_clips_for_drill:
                if ac and not ac.is_playing: # Avoid closing if it's currently active (shouldn't be the case here)
                    ac.close()

        # 4. Set the video clip duration and attach the combined audio
        if final_drill_audio:
            # Add a small buffer to the audio duration
            final_audio_duration = final_drill_audio.duration + 0.5 
            clip = clip.set_duration(max(base_clip_duration, final_audio_duration))
            clip = clip.set_audio(final_drill_audio)
        else:
            # If no audio, maintain base duration
            clip = clip.set_duration(base_clip_duration)
...
def clip_video(source_url, start_time, end_time, output_filename):
    """
    Clip a video from a URL and extract a thumbnail.
    """
    check_moviepy()
    
    print(f"[CLIPPER] Clipping video from {source_url} ({start_time}s - {end_time}s)")
    
    with tempfile.TemporaryDirectory() as tmp_dir:
        # 1. Download source video
        import requests
        suffix = os.path.splitext(source_url)[1] or ".mp4"
        if "?" in suffix: suffix = suffix.split("?")[0]
        temp_source = os.path.join(tmp_dir, f"source{suffix}")
        ...
        # 2. Clip using MoviePy
        with VideoFileClip(temp_source) as video:
            # Add small padding for better UX
            actual_start = max(0, start_time - 0.2)
            actual_end = min(video.duration, end_time + 0.2)
            
            subclip = video.subclip(actual_start, actual_end)
            
            output_path = os.path.join(SHORTS_DIR, output_filename)
            subclip.write_videofile(output_path, codec="libx264", audio_codec="aac", logger=None)
            
            # 3. Save thumbnail (midpoint)
            ...
            video.save_frame(thumb_path, t=midpoint)
            
            return {
                "video_path": output_path,
                "thumb_path": thumb_path,
                "video_url": f"/media/shorts/{output_filename}",
                "thumb_url": f"/media/shorts/{thumb_filename}"
            }
